...
const config = {
  ...
  reconcilerWsPort: parseInt(process.env.RECONCILER_WS_PORT || '46702', 10),
  colonies: {
...
app.get('/api/config', (req, res) => {
  ...
  // Browser connects to this server which proxies to ColonyOS
  res.json({
    colonies: {
      host: browserHost,
...
// Proxy ColonyOS API requests
app.use(express.raw({ type: '*/*', limit: '10mb' }));

app.post('/api', (req, res) => {
  const protocol = config.colonies.tls ? httpsRequest : httpRequest;

  const options = {
    hostname: config.colonies.host,
  ...
  proxyReq.on('error', (err) => {
    ...
    res.status(502).json({ error: 'Failed to connect to ColonyOS server' });
  });
...
server.listen(config.port, '0.0.0.0', () => {
  console.log(`Data Logger Web UI running at http://0.0.0.0:${config.port}`);
