...
const config = {
  ...
  wsPort: parseInt(process.env.RECONCILER_WS_PORT || '46702', 10),
  colonyName: process.env.COLONIES_COLONY_NAME || 'dev',
  colonyPrvKey: process.env.COLONIES_COLONY_PRVKEY,
  executorPrvKey: process.env.DATALOGGER_PRVKEY || process.env.COLONIES_PRVKEY,
  executorId: process.env.DATALOGGER_ID || process.env.COLONIES_EXECUTOR_ID,
};
...
function broadcastDeviceUpdate(deviceName, spec, status) {
  ...
  console.log(`Broadcasting update to ${clients.size} client(s): ${deviceName}`);
...
async function registerExecutor() {
  console.log('Registering data-logger-reconciler executor...');
  ...
  try {
    const executor = await client.getExecutor(config.colonyName, 'data-logger-reconciler');
  ...
  try {
    await client.addExecutor({
      executorname: 'data-logger-reconciler',
      executortype: 'data-logger-reconciler',
      colonyname: config.colonyName,
      executorId: executorId,
    });
    await client.approveExecutor(config.colonyName, 'data-logger-reconciler');
...
async function loadExistingDeviceStates() {
  ...
  try {
    ...
    if (blueprints && blueprints.length > 0) {
      ...
      console.log('No existing data loggers found');
    }
  } catch (error) {
...
async function simulateDevice(deviceName, spec) {
  ...
  // For simulation, we copy spec to status and add runtime fields
  const newStatus = {
    ...spec,
    online: true,
    lastUpdated: new Date().toISOString(),
    uptime: Math.floor(Math.random() * 86400),
    logsCollected: Math.floor(Math.random() * 10000)
  };
...
// Check if spec and status are in sync (ignoring runtime fields)
function needsReconciliation(spec, status) {
  if (!status || Object.keys(status).length === 0) return true;

  // Compare spec fields (flat structure)
  const specFields = ['name', 'location', 'appName', 'appVersion', 'enabled', 'logInterval', 'logFormat'];
  for (const field of specFields) {
    if (spec[field] !== status[field]) {
      return true;
    }
  }
  return false;
...
async function reconcileSingleBlueprint(blueprintName, forceUpdate = false) {
  const t0 = Date.now();
  ...
  // Check if reconciliation is needed (skip check if forced)
  const needsUpdate = needsReconciliation(spec, status);
  if (!needsUpdate && !forceUpdate) {
    console.log(`  [${t1}] Skipping ${blueprintName} - already in sync`);
    return;
  }

  console.log(`  [${t1}] Reconciling device: ${blueprintName} (+${t1-t0}ms)${forceUpdate ? ' [forced]' : ''}`);
  console.log(`    spec: ${spec.appName} v${spec.appVersion}, enabled=${spec.enabled}`);

  // Simulate applying to device
  const newStatus = await simulateDevice(blueprintName, spec);
  const t2 = Date.now();

  console.log(`  [${t2}] Broadcast sent (+${t2-t1}ms)`);
...
async function handleReconcileProcess(process) {
  ...
  try {
    if (blueprintName) {
      // Specific blueprint reconciliation - always force update to notify UI
      await reconcileSingleBlueprint(blueprintName, true);
...
async function reconcileLoop() {
  ...
  console.log(`  TLS: ${config.colonies.tls}`);
  console.log(`  Colony: ${config.colonyName}`);
  console.log(`  Executor key (first 10): ${config.executorPrvKey?.slice(0, 10)}...`);
  ...
  while (true) {
    try {
      ...
      console.log(`[${new Date().toISOString()}] Calling assign...`);
      ...
      if (process) {
        ...
        console.log(`[${new Date().toISOString()}] Assign returned null/undefined`);
      }
    } catch (error) {
      ...
      if (!isExpected) {
        ...
        console.log(`[${new Date().toISOString()}] ${error.message.slice(0, 50)}`);
      }
    }
  }
}
...
async function main() {
  ...
  console.log('Data Logger Reconciler');
  console.log('='.repeat(50));
  console.log(`Colony: ${config.colonyName}`);
